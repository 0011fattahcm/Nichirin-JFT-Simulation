...
export const startSimulation = async (req, res) => {
  ...
  try {
    ...
    if (isValidSimulation) {
      ...
      for (const sesi in questionIds) {
        soal[sesi] = allQuestions.filter((q) =>
          questionIds[sesi].some((id) => id.toString() === q._id.toString())
        );
      }
    ...
    if (user.mockupQuota <= 0) {
      return res.status(403).json({ message: "Kuota tes habis" });
    }

    const historyIds = new Set(
      user.testHistory?.flatMap((t) => t.questionIds || []) || []
    );
    ...
    await logActivity({
      ...
      details: `Quota tersisa: ${user.mockupQuota}`,
    });
